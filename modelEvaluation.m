classdef modelEvaluation < handle
    ...
    methods (Static, Access = private)
        ...
        function eval = evaluate( thisModel, thisDataset )
            ...
            [ eval.XC, offsets ] = thisModel.latentComponents( ...
                            eval.Z, ...
                            sampling = 'Fixed', ...
                            centre = false, ...
                            convert = true );
            ...
            XCFd = smooth_basis( thisDataset.tSpan.target, ...
                                 eval.XC, ...
                                 thisDataset.fda.fdParamsTarget );
            eval.XCSmoothed = squeeze( ...
                        eval_fd( thisDataset.tSpan.target, XCFd ) );
            
            eval.XCRegular = squeeze( ...
                        eval_fd( thisDataset.tSpan.regular, XCFd ) );

            % compute the components' explained variance
            eval.VarProp = thisModel.explainedVariance( ...
                            eval.XHatRegular, eval.XCRegular, offsets );      
