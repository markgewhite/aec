...
function mustBeValidFdParams( fdParams )
    ...
    assert( isempty(setxor( fieldnames( fdParams ), ...
                {'tSpan', 'nBasis', 'basisOrder', 'penaltyOrder', 'lambda'})), ...
        'Padding must have fields: tSpan, nBasis, basisOrder, penaltyOrder and lambda.');

    assert( isvector( fdParams.tSpan ), ...
                'tSpan is not a vector.' );

    assert( fix( fdParams.nBasis ) == fdParams.nBasis & ...
            fdParams.nBasis > 0, ...
                'nBasis is not a positive integer.' );
