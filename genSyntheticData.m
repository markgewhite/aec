...
function [ X, XFd ] = genSyntheticData( nObs, nDim, basis, beta )
...
spacing = cell( nLevels, 1 );
for i = 1:nLevels
    nCoeff(i) = getnbasis( basisFd{i} );
    range = getbasisrange( basisFd{i} );
    spacing{i} = linspace( range(1), range(2), nCoeff(i) )';
end

X = zeros( nCoeff(1), sum(nObs), nDim );
...
for c = 1:length(nObs)
    ...
    for i = 1:nObs(c)
        ...
        a = a+1;
        X( :, a, : ) = coeff + beta(end)*randSeries( nDim, nCoeff(1) )';
